...
#      logout: erase login cookies
	...
	if ($button_code eq "add") { 
	    push (@button_list,
		  td({-align=>"center"},
		     ["Add more data\n" . $query->br . $query->start_form({-action=>"$gaba_tracker_pl"}) . $query->submit(-name=>"button",-value=>"add") . $query->end_form]));
	    next; 
	}
	if ($button_code eq "logout") { 
	    push (@button_list,
		  td({-align=>"center"},
		     ["Logout\n" . $query->br . $query->start_form({-action=>"$gaba_tracker_pl"}) . $query->submit(-name=>"button",-value=>"logout") . $query->end_form]));
	    next; 
	}
	if ($button_code eq "add_id") { 
	    push (@button_list,
		  td({-align=>"center"},
		     ["Add instructor\n" . $query->br . $query->start_form({-action=>"$gaba_tracker_pl"}) . $query->submit(-name=>"button",-value=>"add_ID") . $query->end_form]));
	    next; 
	}
    }
